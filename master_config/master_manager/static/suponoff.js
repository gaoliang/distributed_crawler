...
function update_process(server_name, group_name, process) {
    ...
    var query = _.template('div.server#server-<%= server %> div.group#group-<%= group %> div.program#process-<%= program %>',
        {server: server_name.replace(/\./g, '\\.'), group: group_name, program: process.name})
    ...
    if ('resources' in process) {
        ...
        if ('fileno' in process.resources) {
            if ('max_fileno' in process.resources) {
                fileno_div.find('.textual-value').text(_.template('<%= current %> / <%= max %>', {
                    current: process.resources.fileno,
                    max: process.resources.max_fileno
                }))
                fileno_div.find('.progress-bar').attr('aria-valuenow', process.resources.fileno).attr('aria-valuemax', process.resources.max_fileno).css('width', (process.resources.fileno * 100 / process.resources.max_fileno) + '%')
                fileno_div.find('.progress').show()
            } else {
                fileno_div.find('.textual-value').text(_.template('<%= current %>', {current: process.resources.fileno}))
            }
            fileno_div.find('.textual-value').attr('title', _.template('<%= files %> open files, <%= connections %> sockets', {
                files: process.resources.numfiles,
                connections: process.resources.numconnections
            }))
            fileno_div.show()
        } else {
            fileno_div.hide()
        ...
        if ('vmsize' in process.resources) {
            if ('max_vmsize' in process.resources) {
                vmsize_div.find('.textual-value').text(_.template('<%= current %> / <%= max %>', {
                    current: getReadableFileSizeString(process.resources.vmsize),
                    max: getReadableFileSizeString(process.resources.max_vmsize)
                }))
                vmsize_div.find('.progress-bar').attr('aria-valuenow', process.resources.vmsize).attr('aria-valuemax', process.resources.max_vmsize).css('width', (process.resources.vmsize * 100 / process.resources.max_vmsize) + '%')
